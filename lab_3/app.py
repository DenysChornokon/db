...
# Підключення до PostgreSQL 'postgresql://postgres:admin@127.0.0.1:5432/postgres'
...
def update_reader(reader_id):
    ...
    address = data['address']
    phone_number = data['phone_number']
    cur.execute("CALL update_reader(%s, %s, %s, %s)", (reader_id, name, address, phone_number))
    conn.commit()
...
def create_book():
    ...
    pages = data['pages']
    cur.execute("CALL add_book(%s, %s, %s, %s, %s)", (title, author, genre, publication_year, pages))
    conn.commit()
